...
def test_distribution_dynamic(reaper_reward_distributor_mocked, reaper_mock, ZERO_ADDRESS, lp_token, waves_token, usdn_token, farm_token, neo, morpheus, trinity, thomas, amount, week):
    ...
    tx = reaper_mock.deposit(1 * amount, {'from': morpheus})

    initial_deposit_ts_morpheus = tx.timestamp

    assert initial_deposit_ts_morpheus == reaper_mock.lastReapTimestampFor(morpheus)
    assert reaper_mock.lastReapTimestampFor(neo) == 0
    assert reaper_mock.lastReapTimestampFor(trinity) == 0
    assert initial_deposit_ts_morpheus == reaper_mock.lastReapTimestamp()
    initial_reap_integral = reaper_mock.reapIntegral()
    initial_reap_integral_ts = initial_deposit_ts_morpheus

    # add some reward in usdn
    usdn_token.deposit(reaper_reward_distributor_mocked, 1 * amount, {'from': neo})
    assert usdn_token.balanceOf(reaper_reward_distributor_mocked) == 1 * amount

    # add some reward in waves
    waves_token.deposit(reaper_reward_distributor_mocked, 10 * amount, {'from': neo})
    assert waves_token.balanceOf(reaper_reward_distributor_mocked) == 10 * amount

    # wait for 1 week
    brownie.chain.mine(1, init_ts + week - 50)
    ...
    brownie.chain.mine(1, init_ts + week)

    tx = reaper_mock.deposit(2 * amount, {'from': trinity})
    initial_deposit_ts_trinity = tx.timestamp

    # add some reward in usdn
    usdn_token.deposit(reaper_reward_distributor_mocked, 9 * amount, {'from': neo})
    assert usdn_token.balanceOf(reaper_reward_distributor_mocked) == 9 * amount

    # add some reward in waves
    waves_token.deposit(reaper_reward_distributor_mocked, 90 * amount, {'from': neo})
    assert waves_token.balanceOf(reaper_reward_distributor_mocked) == 90 * amount
    ...
    # assert reaper_reward_distributor_mocked.claimableTokens(usdn_token, morpheus) == 8 * amount
    # assert reaper_reward_distributor_mocked.claimableTokens(waves_token, morpheus) == 100 * amount

    brownie.chain.mine(1, init_ts + 2 * week)
    reaper_mock.snapshot(morpheus)

    assert abs(reaper_mock.reapIntegralFor(morpheus) / reaper_mock.reapIntegral() - 1/2) == 0 # <= 0.00001
    
    assert usdn_token.balanceOf(morpheus) == 1 * amount
    assert abs(reaper_reward_distributor_mocked.claimableTokens(usdn_token, morpheus) - 3 * amount) <= 10 ** 7
    assert abs(reaper_reward_distributor_mocked.claimableTokens(waves_token, morpheus) - 30 * amount) <= 10 ** 8
    # reaper_reward_distributor_mocked.claim(usdn_token, morpheus, ZERO_ADDRESS, {'from': morpheus})
    # assert abs(usdn_token.balanceOf(morpheus) - 4 * amount) <= 10 ** 7

    brownie.chain.mine(1, init_ts + 2 * week)
    reaper_mock.snapshot(trinity)

    assert abs(reaper_mock.reapIntegralFor(trinity) / reaper_mock.reapIntegral() - 1/2) <= 0.00001
    assert reaper_reward_distributor_mocked.claimableTokens(usdn_token, trinity) == 5 * amount
    assert reaper_reward_distributor_mocked.claimableTokens(waves_token, trinity) == 50 * amount
