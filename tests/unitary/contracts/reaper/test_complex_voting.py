...
def test_complex_voting(farm_token, lp_token, reaper, reaper_2, controller, voting_controller, deployer, morpheus, trinity, thomas, MAX_UINT256, chain, year, day):
    ...
    # assert reaper.balancesIntegral() == amount * (tx3.timestamp - tx1.timestamp) #+ amount * (tx2.timestamp - tx1.timestamp)
    ...
    # 1st snapshot for both on reaper
    chain.mine(1, init_ts + 8 * day)
    tx4 = reaper.snapshot({'from': deployer})
    # chain.mine(1, init_ts + 8 * day)
    # tx4 = reaper.snapshot({'from': morpheus})
    assert reaper.balances(deployer) == amount
    # assert reaper.totalBalances() == 2 * amount
    # assert reaper.balancesIntegral() == amount * (tx4.timestamp - tx3.timestamp) #+ last_balances_integral
    # assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    # assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    # assert reaper.emissionIntegral() == YEAR_EMISSION * (tx4.timestamp - tx_emission.timestamp) // year
    # assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_intergal) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral)
    ...
    assert reaper.voteIntegral() == (tx4.timestamp - voting_controller.lastSnapshotTimestamp()) * VOTE_DIVIDER // 2 # reaper share=50%
    assert reaper.voteIntegral() == voting_controller.voteIntegral(reaper)
    assert reaper.voteIntegral() / (tx4.timestamp - voting_controller.lastSnapshotTimestamp()) / VOTE_DIVIDER == 0.5
    assert reaper.boostIntegralFor(deployer) == 0
    assert reaper.totalBoostIntegralFor(deployer) == 0
    assert reaper.boostIntegralFor(morpheus) == 0
    assert reaper.totalBoostIntegralFor(morpheus) == 0
