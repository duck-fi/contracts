...
def test_complex_voting(farm_token, lp_token, reaper, reaper_2, reaper_3, controller, voting_controller, deployer, morpheus, trinity, thomas, MAX_UINT256, chain, year, week, day, amount):
    # amount = 10**20
    ...
    voting_controller.vote(reaper, farm_token, 1, {'from': deployer}) # TODO: remove TODO: add one more test
    voting_controller.vote(reaper_2, farm_token, 1, {'from': deployer})

    # wait for 7 days
    # startVoting + startEmission
    aligned_time = INIT_VOTING_TIME + ((chain.time() + week - INIT_VOTING_TIME) // week) * week + 1
    while True:
        ...
        if tx_voting.timestamp == tx_emission.timestamp:
            break
        else:
            chain.undo(2)
    ...
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx4.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx4.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - 0)
    ...
    assert reaper.voteIntegral() == (tx4.timestamp - tx_voting.timestamp) * VOTE_DIVIDER // 2 # reaper share=50%
    assert reaper.voteIntegral() / (tx4.timestamp - tx_voting.timestamp) / VOTE_DIVIDER == 0.5
    assert reaper.boostIntegralFor(deployer) == 0
    assert reaper.totalBoostIntegralFor(deployer) == 0
    assert reaper.boostIntegralFor(morpheus) == 0
    assert reaper.totalBoostIntegralFor(morpheus) == 0
    ...
    voting_controller.vote(reaper_2, farm_token, 2, {'from': deployer}) # TODO: 3
    ...
    assert reaper.balances(morpheus) == amount
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx5.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx5.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral) + last_unit_cost_integral
    assert reaper.lastUnitCostIntegralFor(deployer) == reaper.unitCostIntegral()
    assert reaper.lastUnitCostIntegralFor(morpheus) == reaper.unitCostIntegral()
    assert reaper.reapIntegral() == reaper.reapIntegralFor(deployer) + reaper.reapIntegralFor(morpheus)
    voting_koeff = reaper.voteIntegral() / (tx5.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff)
    assert abs(reaper.reapIntegral() - reaper.emissionIntegral() // 2 * voting_koeff) <= 10 ** 8 # reaper share=voting_koeff, no boosts
    ...
    assert reaper.reapIntegralFor(morpheus) == (reaper.lastUnitCostIntegralFor(morpheus) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_morpheus
    assert reaper.reapIntegralFor(deployer) == reaper.reapIntegralFor(morpheus)
    ...
    assert abs(reaper.voteIntegral() / (tx5.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.375) < 10 ** (-6)
    ...
    assert reaper.balances(morpheus) == amount
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx6.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx6.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral) + last_unit_cost_integral
    assert reaper.lastUnitCostIntegralFor(deployer) == reaper.unitCostIntegral()
    assert reaper.lastUnitCostIntegralFor(morpheus) == reaper.unitCostIntegral()
    assert reaper.reapIntegral() == reaper.reapIntegralFor(deployer) + reaper.reapIntegralFor(morpheus)
    voting_koeff = reaper.voteIntegral() / (tx6.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff)
    assert abs(reaper.reapIntegral() - reaper.emissionIntegral() // 2 * voting_koeff) <= 10 ** 8 # reaper share=voting_koeff, no boosts
    assert reaper.reapIntegralFor(deployer) == (reaper.lastUnitCostIntegralFor(deployer) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_deployer
    assert reaper.reapIntegralFor(morpheus) == (reaper.lastUnitCostIntegralFor(morpheus) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_morpheus
    ...
    assert abs(reaper.voteIntegral() / (tx6.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.333) < 10 ** (-3)
    ...
    assert reaper.lastSnapshotTimestampFor(deployer) == tx6.timestamp
    assert reaper.lastSnapshotTimestampFor(morpheus) == tx6.timestamp
    assert abs(reaper.voteIntegral() / (tx6.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.357) < 10 ** (-3)
    assert voting_controller.lastVotes(reaper) == 0.5 * 10 ** 18
    assert reaper.boostIntegralFor(deployer) == 0
    assert reaper.totalBoostIntegralFor(deployer) == 0
    assert reaper.boostIntegralFor(morpheus) == 0
    ...
    assert reaper.lastSnapshotTimestampFor(deployer) == tx6.timestamp
    assert reaper.lastSnapshotTimestampFor(morpheus) == tx6.timestamp
    assert abs(reaper.voteIntegral() / (tx6.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.375) < 10 ** (-3)
    assert voting_controller.lastVotes(reaper) == 0.5 * 10 ** 18
    assert reaper.boostIntegralFor(deployer) == 0
    assert reaper.totalBoostIntegralFor(deployer) == 0
    assert reaper.boostIntegralFor(morpheus) == 0
    ...
    assert reaper_2.balancesIntegral() == 2 * amount * (tx6.timestamp - tx_emission.timestamp)
    assert reaper_2.balancesIntegralFor(deployer) == reaper_2.balancesIntegral()
    assert reaper_2.balancesIntegralFor(morpheus) == reaper_2.balancesIntegral()
    assert reaper_2.emissionIntegral() == YEAR_EMISSION * (tx6.timestamp - tx_emission.timestamp) // year
    assert reaper_2.unitCostIntegral() == (reaper_2.emissionIntegral() - 0) * (reaper_2.voteIntegral() - 0) // (reaper_2.balancesIntegral() - 0) + 0
    assert reaper_2.lastUnitCostIntegralFor(deployer) == reaper_2.unitCostIntegral()
    assert reaper_2.lastUnitCostIntegralFor(morpheus) == reaper_2.unitCostIntegral()
    assert reaper_2.reapIntegral() == reaper_2.reapIntegralFor(deployer) + reaper_2.reapIntegralFor(morpheus)
    voting_koeff_2 = reaper_2.voteIntegral() / (tx6.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff_2)
    assert abs(reaper_2.reapIntegral() - reaper_2.emissionIntegral() // 2 * voting_koeff_2) <= 10 ** 8 # reaper_2 share=voting_koeff_2, no boosts
    assert reaper_2.reapIntegralFor(deployer) == (reaper_2.lastUnitCostIntegralFor(deployer) - 0) * amount // 2 // VOTE_DIVIDER + 0
    assert reaper_2.reapIntegralFor(morpheus) == (reaper_2.lastUnitCostIntegralFor(morpheus) - 0) * amount // 2 // VOTE_DIVIDER + 0
    assert reaper_2.reapIntegralFor(deployer) == reaper_2.reapIntegralFor(morpheus)
    ...
    assert abs(reaper_2.voteIntegral() / (tx6.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.625) < 10 ** (-3)
    ...
    assert abs(reaper_2.reapIntegral() + reaper.reapIntegral() - reaper_2.emissionIntegral() // 2) < 10 ** 8
    ...
    assert reaper.balances(morpheus) == amount
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx8.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx8.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral) + last_unit_cost_integral
    assert reaper.lastUnitCostIntegralFor(deployer) == reaper.unitCostIntegral()
    assert reaper.lastUnitCostIntegralFor(morpheus) == reaper.unitCostIntegral()
    assert reaper.reapIntegral() == reaper.reapIntegralFor(deployer) + reaper.reapIntegralFor(morpheus)
    voting_koeff = reaper.voteIntegral() / (tx8.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff)
    assert abs(reaper.reapIntegral() - reaper.emissionIntegral() // 2 * voting_koeff) <= 10 ** 8 # reaper share=voting_koeff, no boosts
    distributed_emission_reaper = (reaper.unitCostIntegral() - last_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper.reapIntegralFor(deployer) == (reaper.lastUnitCostIntegralFor(deployer) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_deployer
    assert reaper.reapIntegralFor(morpheus) == (reaper.lastUnitCostIntegralFor(morpheus) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_morpheus
    assert reaper.reapIntegralFor(deployer) == reaper.reapIntegralFor(morpheus)
    ...
    assert abs(reaper.voteIntegral() / (tx8.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.366) < 10 ** (-3)
    ...
    assert reaper_2.balances(morpheus) == amount
    assert reaper_2.totalBalances() == 2 * amount
    assert reaper_2.balancesIntegral() == 2 * amount * (tx9.timestamp - tx_emission.timestamp)
    assert reaper_2.balancesIntegralFor(deployer) == reaper_2.balancesIntegral()
    assert reaper_2.balancesIntegralFor(morpheus) == reaper_2.balancesIntegral()
    assert reaper_2.emissionIntegral() == YEAR_EMISSION * (tx9.timestamp - tx_emission.timestamp) // year
    assert reaper_2.unitCostIntegral() == (reaper_2.emissionIntegral() - last_reaper_2_emission_integral) * (reaper_2.voteIntegral() - last_reaper_2_vote_integral) // (reaper_2.balancesIntegral() - last_reaper_2_balances_integral) + last_reaper_2_unit_cost_integral
    assert reaper_2.lastUnitCostIntegralFor(deployer) == reaper_2.unitCostIntegral()
    assert reaper_2.lastUnitCostIntegralFor(morpheus) == reaper_2.unitCostIntegral()
    assert reaper_2.reapIntegral() == reaper_2.reapIntegralFor(deployer) + reaper_2.reapIntegralFor(morpheus)
    voting_koeff_2 = reaper_2.voteIntegral() / (tx9.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff_2)
    assert abs(reaper_2.reapIntegral() - reaper_2.emissionIntegral() // 2 * voting_koeff_2) <= 10 ** 8 # reaper_2 share=voting_koeff_2, no boosts
    distributed_emission_reaper_2 = (reaper_2.unitCostIntegral() - last_reaper_2_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_2.reapIntegralFor(deployer) == (reaper_2.lastUnitCostIntegralFor(deployer) - last_reaper_2_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_2_reap_integral_deployer
    assert reaper_2.reapIntegralFor(morpheus) == (reaper_2.lastUnitCostIntegralFor(morpheus) - last_reaper_2_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_2_reap_integral_morpheus
    assert reaper_2.reapIntegralFor(deployer) == reaper_2.reapIntegralFor(morpheus)
    ...
    assert abs(reaper_2.voteIntegral() / (tx9.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.566) < 10 ** (-3)
    ...
    assert reaper_3.balancesIntegral() == 2 * amount * (tx10.timestamp - tx7.timestamp)
    assert reaper_3.balancesIntegralFor(deployer) == reaper_3.balancesIntegral()
    assert reaper_3.balancesIntegralFor(morpheus) == reaper_3.balancesIntegral()
    assert reaper_3.emissionIntegral() == YEAR_EMISSION * (tx10.timestamp - tx_emission.timestamp) // year
    assert reaper_3.unitCostIntegral() == (reaper_3.emissionIntegral() - init_reaper_3_emission_integral) * (reaper_3.voteIntegral() - 0) // (reaper_3.balancesIntegral() - 0) + 0
    assert reaper_3.lastUnitCostIntegralFor(deployer) == reaper_3.unitCostIntegral()
    assert reaper_3.lastUnitCostIntegralFor(morpheus) == reaper_3.unitCostIntegral()
    assert reaper_3.reapIntegral() == reaper_3.reapIntegralFor(deployer) + reaper_3.reapIntegralFor(morpheus)
    voting_koeff_3 = reaper_3.voteIntegral() / (tx10.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER
    print(voting_koeff_3)
    assert abs(reaper_3.reapIntegral() - (reaper_3.emissionIntegral() - init_reaper_3_emission_integral) // 2 * voting_koeff_3) <= 10 ** 8 # reaper_3 share=voting_koeff_3, no boosts
    distributed_emission_reaper_3 = (reaper_3.unitCostIntegral() - 0) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_3.reapIntegralFor(deployer) == (reaper_3.lastUnitCostIntegralFor(deployer) - 0) * amount // 2 // VOTE_DIVIDER + 0
    assert reaper_3.reapIntegralFor(morpheus) == (reaper_3.lastUnitCostIntegralFor(morpheus) - 0) * amount // 2 // VOTE_DIVIDER + 0
    assert reaper_3.reapIntegralFor(deployer) == reaper_3.reapIntegralFor(morpheus)
    ...
    voting_koeff_3_full = reaper_3.voteIntegral() / (tx10.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    assert abs(reaper_3.voteIntegral() / (tx10.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.066) < 10 ** (-3)
    assert abs(reaper_3.voteIntegral() / (tx10.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER - 0.333) < 10 ** (-3)
    ...
    distributed_emission = (reaper_3.emissionIntegral() - previous_emission_integral) // 2
    previous_emission_integral = reaper_3.emissionIntegral() 
    print("--------")
    ...
    print("--------")
    assert abs(distributed_emission - distributed_emission_reaper - distributed_emission_reaper_2 - distributed_emission_reaper_3) < 10 ** 8
    assert abs(voting_koeff + voting_koeff_2 + voting_koeff_3_full - 1) < 10 ** (-3)
    ...
    assert reaper.balances(morpheus) == amount
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx11.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx11.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral) + last_unit_cost_integral
    assert reaper.lastUnitCostIntegralFor(deployer) == reaper.unitCostIntegral()
    assert reaper.lastUnitCostIntegralFor(morpheus) == reaper.unitCostIntegral()
    assert reaper.reapIntegral() == reaper.reapIntegralFor(deployer) + reaper.reapIntegralFor(morpheus)
    voting_koeff = reaper.voteIntegral() / (tx11.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff)
    assert abs(reaper.reapIntegral() - reaper.emissionIntegral() // 2 * voting_koeff) <= 10 ** 8 # reaper share=voting_koeff, no boosts
    distributed_emission_reaper = (reaper.unitCostIntegral() - last_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper.reapIntegralFor(deployer) == (reaper.lastUnitCostIntegralFor(deployer) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_deployer
    ...
    assert abs(reaper.voteIntegral() / (tx11.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.347) < 10 ** (-3)
    ...
    assert reaper_2.balances(morpheus) == amount
    assert reaper_2.totalBalances() == 2 * amount
    assert reaper_2.balancesIntegral() == 2 * amount * (tx12.timestamp - tx_emission.timestamp)
    assert reaper_2.balancesIntegralFor(deployer) == reaper_2.balancesIntegral()
    assert reaper_2.balancesIntegralFor(morpheus) == reaper_2.balancesIntegral()
    assert reaper_2.emissionIntegral() == YEAR_EMISSION * (tx12.timestamp - tx_emission.timestamp) // year
    assert reaper_2.unitCostIntegral() == (reaper_2.emissionIntegral() - last_reaper_2_emission_integral) * (reaper_2.voteIntegral() - last_reaper_2_vote_integral) // (reaper_2.balancesIntegral() - last_reaper_2_balances_integral) + last_reaper_2_unit_cost_integral
    assert reaper_2.lastUnitCostIntegralFor(deployer) == reaper_2.unitCostIntegral()
    assert reaper_2.lastUnitCostIntegralFor(morpheus) == reaper_2.unitCostIntegral()
    assert reaper_2.reapIntegral() == reaper_2.reapIntegralFor(deployer) + reaper_2.reapIntegralFor(morpheus)
    voting_koeff_2 = reaper_2.voteIntegral() / (tx12.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff_2)
    assert abs(reaper_2.reapIntegral() - reaper_2.emissionIntegral() // 2 * voting_koeff_2) <= 10 ** 8 # reaper_2 share=voting_koeff_2, no boosts
    distributed_emission_reaper_2 = (reaper_2.unitCostIntegral() - last_reaper_2_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_2.reapIntegralFor(deployer) == (reaper_2.lastUnitCostIntegralFor(deployer) - last_reaper_2_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_2_reap_integral_deployer
    assert reaper_2.reapIntegralFor(morpheus) == (reaper_2.lastUnitCostIntegralFor(morpheus) - last_reaper_2_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_2_reap_integral_deployer
    assert reaper_2.reapIntegralFor(deployer) == reaper_2.reapIntegralFor(morpheus)
    ...
    assert abs(reaper_2.voteIntegral() / (tx12.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.513) < 10 ** (-3)
    ...
    assert reaper_3.balances(morpheus) == amount
    assert reaper_3.totalBalances() == 2 * amount
    assert reaper_3.balancesIntegral() == 2 * amount * (tx13.timestamp - tx7.timestamp)
    assert reaper_3.balancesIntegralFor(deployer) == reaper_3.balancesIntegral()
    assert reaper_3.balancesIntegralFor(morpheus) == reaper_3.balancesIntegral()
    assert reaper_3.emissionIntegral() == YEAR_EMISSION * (tx13.timestamp - tx_emission.timestamp) // year
    assert reaper_3.unitCostIntegral() == (reaper_3.emissionIntegral() - last_reaper_3_emission_integral) * (reaper_3.voteIntegral() - last_reaper_3_vote_integral) // (reaper_3.balancesIntegral() - last_reaper_3_balances_integral) + last_reaper_3_unit_cost_integral
    assert reaper_3.lastUnitCostIntegralFor(deployer) == reaper_3.unitCostIntegral()
    assert reaper_3.lastUnitCostIntegralFor(morpheus) == reaper_3.unitCostIntegral()
    assert reaper_3.reapIntegral() == reaper_3.reapIntegralFor(deployer) + reaper_3.reapIntegralFor(morpheus)
    voting_koeff_3 = reaper_3.voteIntegral() / (tx13.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER
    print(voting_koeff_3)
    assert abs(reaper_3.reapIntegral() - (reaper_3.emissionIntegral() - init_reaper_3_emission_integral) // 2 * voting_koeff_3) <= 10 ** 8 # reaper_3 share=voting_koeff_3, no boosts
    distributed_emission_reaper_3 = (reaper_3.unitCostIntegral() - last_reaper_3_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_3.reapIntegralFor(deployer) == (reaper_3.lastUnitCostIntegralFor(deployer) - last_reaper_3_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_3_reap_integral_deployer
    assert reaper_3.reapIntegralFor(morpheus) == (reaper_3.lastUnitCostIntegralFor(morpheus) - last_reaper_3_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_3_reap_integral_deployer
    assert reaper_3.reapIntegralFor(deployer) == reaper_3.reapIntegralFor(morpheus)
    ...
    voting_koeff_3_full = reaper_3.voteIntegral() / (tx13.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    assert abs(reaper_3.voteIntegral() / (tx13.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.138) < 10 ** (-3)
    assert abs(reaper_3.voteIntegral() / (tx13.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER - 0.416) < 10 ** (-3)
    ...
    assert reaper.balances(morpheus) == amount
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx14.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx14.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral) + last_unit_cost_integral
    assert reaper.lastUnitCostIntegralFor(deployer) == reaper.unitCostIntegral()
    assert reaper.lastUnitCostIntegralFor(morpheus) == reaper.unitCostIntegral()
    assert reaper.reapIntegral() == reaper.reapIntegralFor(deployer) + reaper.reapIntegralFor(morpheus)
    voting_koeff = reaper.voteIntegral() / (tx14.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff)
    assert abs(reaper.reapIntegral() - reaper.emissionIntegral() // 2 * voting_koeff) <= 10 ** 8 # reaper share=voting_koeff, no boosts
    distributed_emission_reaper = (reaper.unitCostIntegral() - last_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper.reapIntegralFor(deployer) == (reaper.lastUnitCostIntegralFor(deployer) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_deployer
    ...
    assert abs(reaper.voteIntegral() / (tx14.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.369) < 10 ** (-3)
    ...
    assert reaper_2.balances(morpheus) == amount
    assert reaper_2.totalBalances() == 2 * amount
    assert reaper_2.balancesIntegral() == 2 * amount * (tx15.timestamp - tx_emission.timestamp)
    assert reaper_2.balancesIntegralFor(deployer) == reaper_2.balancesIntegral()
    assert reaper_2.balancesIntegralFor(morpheus) == reaper_2.balancesIntegral()
    assert reaper_2.emissionIntegral() == YEAR_EMISSION * (tx15.timestamp - tx_emission.timestamp) // year
    assert reaper_2.unitCostIntegral() == (reaper_2.emissionIntegral() - last_reaper_2_emission_integral) * (reaper_2.voteIntegral() - last_reaper_2_vote_integral) // (reaper_2.balancesIntegral() - last_reaper_2_balances_integral) + last_reaper_2_unit_cost_integral
    assert reaper_2.lastUnitCostIntegralFor(deployer) == reaper_2.unitCostIntegral()
    assert reaper_2.lastUnitCostIntegralFor(morpheus) == reaper_2.unitCostIntegral()
    assert reaper_2.reapIntegral() == reaper_2.reapIntegralFor(deployer) + reaper_2.reapIntegralFor(morpheus)
    voting_koeff_2 = reaper_2.voteIntegral() / (tx15.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff_2)
    assert abs(reaper_2.reapIntegral() - reaper_2.emissionIntegral() // 2 * voting_koeff_2) <= 10 ** 8 # reaper_2 share=voting_koeff_2, no boosts
    distributed_emission_reaper_2 = (reaper_2.unitCostIntegral() - last_reaper_2_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_2.reapIntegralFor(deployer) == (reaper_2.lastUnitCostIntegralFor(deployer) - last_reaper_2_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_2_reap_integral_deployer
    ...
    assert abs(reaper_2.voteIntegral() / (tx15.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.511) < 10 ** (-3)
    ...
    assert reaper_3.balances(morpheus) == amount
    assert reaper_3.totalBalances() == 2 * amount
    assert reaper_3.balancesIntegral() == 2 * amount * (tx16.timestamp - tx7.timestamp)
    assert reaper_3.balancesIntegralFor(deployer) == reaper_3.balancesIntegral()
    assert reaper_3.balancesIntegralFor(morpheus) == reaper_3.balancesIntegral()
    assert reaper_3.emissionIntegral() == YEAR_EMISSION * (tx16.timestamp - tx_emission.timestamp) // year
    assert reaper_3.unitCostIntegral() == (reaper_3.emissionIntegral() - last_reaper_3_emission_integral) * (reaper_3.voteIntegral() - last_reaper_3_vote_integral) // (reaper_3.balancesIntegral() - last_reaper_3_balances_integral) + last_reaper_3_unit_cost_integral
    assert reaper_3.lastUnitCostIntegralFor(deployer) == reaper_3.unitCostIntegral()
    assert reaper_3.lastUnitCostIntegralFor(morpheus) == reaper_3.unitCostIntegral()
    assert reaper_3.reapIntegral() == reaper_3.reapIntegralFor(deployer) + reaper_3.reapIntegralFor(morpheus)
    voting_koeff_3 = reaper_3.voteIntegral() / (tx16.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER
    print(voting_koeff_3)
    assert abs(reaper_3.reapIntegral() - (reaper_3.emissionIntegral() - init_reaper_3_emission_integral) // 2 * voting_koeff_3) <= 10 ** 8 # reaper_3 share=voting_koeff_3, no boosts
    distributed_emission_reaper_3 = (reaper_3.unitCostIntegral() - last_reaper_3_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_3.reapIntegralFor(deployer) == (reaper_3.lastUnitCostIntegralFor(deployer) - last_reaper_3_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_3_reap_integral_deployer
    ...
    voting_koeff_3_full = reaper_3.voteIntegral() / (tx16.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    assert abs(reaper_3.voteIntegral() / (tx16.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.119) < 10 ** (-3)
    assert abs(reaper_3.voteIntegral() / (tx16.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER - 0.277) < 10 ** (-3)
    ...
    assert reaper.balances(morpheus) == amount
    assert reaper.totalBalances() == 2 * amount
    assert reaper.balancesIntegral() == 2 * amount * (tx17.timestamp - tx_emission.timestamp)
    assert reaper.balancesIntegralFor(deployer) == reaper.balancesIntegral()
    assert reaper.balancesIntegralFor(morpheus) == reaper.balancesIntegral()
    assert reaper.emissionIntegral() == YEAR_EMISSION * (tx17.timestamp - tx_emission.timestamp) // year
    assert reaper.unitCostIntegral() == (reaper.emissionIntegral() - last_emission_integral) * (reaper.voteIntegral() - last_vote_integral) // (reaper.balancesIntegral() - last_balances_integral) + last_unit_cost_integral
    assert reaper.lastUnitCostIntegralFor(deployer) == reaper.unitCostIntegral()
    assert reaper.lastUnitCostIntegralFor(morpheus) == reaper.unitCostIntegral()
    assert reaper.reapIntegral() == reaper.reapIntegralFor(deployer) + reaper.reapIntegralFor(morpheus)
    voting_koeff = reaper.voteIntegral() / (tx17.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff)
    assert abs(reaper.reapIntegral() - reaper.emissionIntegral() // 2 * voting_koeff) <= 10 ** 8 # reaper share=voting_koeff, no boosts
    distributed_emission_reaper = (reaper.unitCostIntegral() - last_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper.reapIntegralFor(deployer) == (reaper.lastUnitCostIntegralFor(deployer) - last_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reap_integral_deployer
    ...
    assert abs(reaper.voteIntegral() / (tx17.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.385) < 10 ** (-3)
    assert voting_controller.lastVotes(reaper) == 500000000000000000
    assert reaper.boostIntegralFor(deployer) == 0
    assert reaper.totalBoostIntegralFor(deployer) == 0
    assert reaper.boostIntegralFor(morpheus) == 0
    ...
    assert reaper_2.balances(morpheus) == amount
    assert reaper_2.totalBalances() == 2 * amount
    assert reaper_2.balancesIntegral() == 2 * amount * (tx18.timestamp - tx_emission.timestamp)
    assert reaper_2.balancesIntegralFor(deployer) == reaper_2.balancesIntegral()
    assert reaper_2.balancesIntegralFor(morpheus) == reaper_2.balancesIntegral()
    assert reaper_2.emissionIntegral() == YEAR_EMISSION * (tx18.timestamp - tx_emission.timestamp) // year
    assert reaper_2.unitCostIntegral() == (reaper_2.emissionIntegral() - last_reaper_2_emission_integral) * (reaper_2.voteIntegral() - last_reaper_2_vote_integral) // (reaper_2.balancesIntegral() - last_reaper_2_balances_integral) + last_reaper_2_unit_cost_integral
    assert reaper_2.lastUnitCostIntegralFor(deployer) == reaper_2.unitCostIntegral()
    assert reaper_2.lastUnitCostIntegralFor(morpheus) == reaper_2.unitCostIntegral()
    assert reaper_2.reapIntegral() == reaper_2.reapIntegralFor(deployer) + reaper_2.reapIntegralFor(morpheus)
    voting_koeff_2 = reaper_2.voteIntegral() / (tx18.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    print(voting_koeff_2)
    assert abs(reaper_2.reapIntegral() - reaper_2.emissionIntegral() // 2 * voting_koeff_2) <= 10 ** 8 # reaper_2 share=voting_koeff_2, no boosts
    distributed_emission_reaper_2 = (reaper_2.unitCostIntegral() - last_reaper_2_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_2.reapIntegralFor(deployer) == (reaper_2.lastUnitCostIntegralFor(deployer) - last_reaper_2_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_2_reap_integral_deployer
    ...
    assert abs(reaper_2.voteIntegral() / (tx18.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.511) < 10 ** (-3)
    assert voting_controller.lastVotes(reaper_2) == 500000000000000000
    assert reaper_2.boostIntegralFor(deployer) == 0
    assert reaper_2.totalBoostIntegralFor(deployer) == 0
    assert reaper_2.boostIntegralFor(morpheus) == 0
    ...
    assert reaper_3.balances(morpheus) == amount
    assert reaper_3.totalBalances() == 2 * amount
    assert reaper_3.balancesIntegral() == 2 * amount * (tx19.timestamp - tx7.timestamp)
    assert reaper_3.balancesIntegralFor(deployer) == reaper_3.balancesIntegral()
    assert reaper_3.balancesIntegralFor(morpheus) == reaper_3.balancesIntegral()
    assert reaper_3.emissionIntegral() == YEAR_EMISSION * (tx19.timestamp - tx_emission.timestamp) // year
    assert reaper_3.unitCostIntegral() == (reaper_3.emissionIntegral() - last_reaper_3_emission_integral) * (reaper_3.voteIntegral() - last_reaper_3_vote_integral) // (reaper_3.balancesIntegral() - last_reaper_3_balances_integral) + last_reaper_3_unit_cost_integral
    assert reaper_3.lastUnitCostIntegralFor(deployer) == reaper_3.unitCostIntegral()
    assert reaper_3.lastUnitCostIntegralFor(morpheus) == reaper_3.unitCostIntegral()
    assert reaper_3.reapIntegral() == reaper_3.reapIntegralFor(deployer) + reaper_3.reapIntegralFor(morpheus)
    voting_koeff_3 = reaper_3.voteIntegral() / (tx19.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER
    print(voting_koeff_3)
    assert abs(reaper_3.reapIntegral() - (reaper_3.emissionIntegral() - init_reaper_3_emission_integral) // 2 * voting_koeff_3) <= 10 ** 8 # reaper_3 share=voting_koeff_3, no boosts
    distributed_emission_reaper_3 = (reaper_3.unitCostIntegral() - last_reaper_3_unit_cost_integral) * amount * 2 // 2 // VOTE_DIVIDER
    assert reaper_3.reapIntegralFor(deployer) == (reaper_3.lastUnitCostIntegralFor(deployer) - last_reaper_3_unit_cost_integral) * amount // 2 // VOTE_DIVIDER + last_reaper_3_reap_integral_deployer
    ...
    voting_koeff_3_full = reaper_3.voteIntegral() / (tx19.timestamp - tx_voting.timestamp) / VOTE_DIVIDER
    assert abs(reaper_3.voteIntegral() / (tx19.timestamp - tx_voting.timestamp) / VOTE_DIVIDER - 0.104) < 10 ** (-3)
    assert abs(reaper_3.voteIntegral() / (tx19.timestamp - reaper_3_init_timestamp.timestamp) / VOTE_DIVIDER - 0.208) < 10 ** (-3)
    assert voting_controller.lastVotes(reaper_3) == 0
    assert reaper_3.boostIntegralFor(deployer) == 0
    assert reaper_3.totalBoostIntegralFor(deployer) == 0
    assert reaper_3.boostIntegralFor(morpheus) == 0
